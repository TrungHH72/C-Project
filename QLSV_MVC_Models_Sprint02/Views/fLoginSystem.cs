            ...
            if (tbUsername.TextLength > 0 && tbPassword.TextLength > 0)
            {
                ...
                if (admFunc.checkAccountValid(tbUsername.Text, tbPassword.Text))
                {
                    ...
                    if (admFunc.dtAc.Rows[0][3].ToString() == "1")
                    {
                        // Ẩn Form đăng nhập
                        this.Hide();
                        getInfoAccount.userid = admFunc.dtAc.Rows[0][0].ToString();
                        getInfoAccount.username = admFunc.dtAc.Rows[0][1].ToString();
                        getInfoAccount.roleid = Convert.ToInt32(admFunc.dtAc.Rows[0][3].ToString());
